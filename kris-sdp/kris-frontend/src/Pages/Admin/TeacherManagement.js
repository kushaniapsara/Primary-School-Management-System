...
const TeacherManagement = () => {
    ...
    setTeachers((prevTeachers) =>
      prevTeachers.filter((teacher) => {
        return (
          ...
          (filters.className === "" ||
            teacher.Class_name === filters.className) &&
          (filters.name === "" ||
            teacher.Full_name.toLowerCase().includes(
              filters.name.toLowerCase()
            )) && // Add name filtering here
          (filters.academicYear === "" ||
            String(teacher.Academic_year) === filters.academicYear) &&
  ...
  const handlePromote = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5001/api/teachers/promote",
        {
          method: "POST",
          headers: {
  ...
  const toggleStatus = async (teacherId, currentStatus) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5001/api/teachers/${teacherId}/status`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
    ...
    if (!value || value.trim() === "") {
      ...
      switch (name) {
        ...
        case "age":
          ...
          else if (parseInt(value) < 18 || parseInt(value) > 60)
  ...
  const submitEditTeacherDetails = async () => {
    try {
      const res = await fetch(
        `http://localhost:5001/api/teachers/details/${selectedTeacher.Teacher_ID}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({
            ...
            Name_with_initials: selectedTeacher.Name_with_initials,
            Age: selectedTeacher.Age,
            Contact_number: selectedTeacher.Contact_number,
            Email: selectedTeacher.Email,
            Documents: selectedTeacher.Documents,
      ...
      if (res.ok) {
        ...
        setTeachers((prev) =>
          prev.map((s) =>
            s.Teacher_ID === selectedTeacher.Teacher_ID
              ? { ...s, ...selectedTeacher }
              : s
          )
        );
  ...
  const submitPwChange = async () => {
    try {
      const response = await fetch(
        `http://localhost:5001/api/teachers/changepw/${selectedTeacher.Teacher_ID}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
              ...
              <div className="bg-white p-6 rounded-lg w-3/4 max-w-4xl">
                ...
                <div className="flex">
                  ...
                  <div className="w-1/3 text-center border-r-2 pr-4">
                    <img
                      src={`http://localhost:5001/${selectedTeacher.Profile_photo}`}
                      alt="Profile"
                    ...
                    {editTeacherMode ? (
                      <>
                        ...
                        <TextField
                          ...
                          value={selectedTeacher.Name_with_initials}
                          onChange={handleTeacherChange}
                        />
                        <TextField
                          fullWidth
                        ...
                        <TextField
                          ...
                          name="Contact_number"
                          value={selectedTeacher.Contact_number}
                          onChange={handleTeacherChange}
                        />
                        <TextField
                          fullWidth
                        ...
                        <TextField
                          ...
                          label="Documents"
                          name="Documents"
                          value={selectedTeacher.Documents}
                          onChange={handleTeacherChange}
                        />
                      </>
                    ) : (
                      <>
                        ...
                        <p>
                          <b>🎂 Age:</b> {selectedTeacher.Age}
                        </p>
                        <p>
                          <b>🏠 Address:</b> {selectedTeacher.Address}
                        </p>
                        <p>
                          <b>📞 Contact:</b> {selectedTeacher.Contact_number} |{" "}
                        ...
                        <p>
                          <b>📂 Documents:</b> {selectedTeacher.Documents}
                        </p>
                      </>
                    )}
                    ...
                    <div className="mt-4 flex justify-between">
                      {!pwChangeMode ? (
                        <button
                          ...
                          onClick={() => setEditTeacherMode((prev) => !prev)}
                        >
                          {!editTeacherMode ? "✏️ Edit" : "Cancel"}
                        </button>
                      ) : (
                        <TextField
                          fullWidth
                      ...
                      {!pwChangeMode && !editTeacherMode ? (
                        <button
                          className={`${
                            editTeacherMode ? "bg-gray-500" : "bg-blue-500"
                      ...
                      {editTeacherMode && (
                        <button
                          ...
                          onClick={submitEditTeacherDetails}
                        >
                          💾 Save
                        </button>
                      )}
        ...
        >
          ...
          <DialogContent>
            ...
            <TextField
              ...
              value={newTeacher.nameWithInitials}
              onChange={handleChange}
            />
            <TextField
              fullWidth
            ...
            <TextField
              ...
              name="enrollmentDate"
              type="date"
              value={newTeacher.enrollmentDate}
              onChange={handleChange}
            ...
            <TextField
              ...
              value={newTeacher.previousSchools}
              onChange={handleChange}
            />
            <TextField
              fullWidth
